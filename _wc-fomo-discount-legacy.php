...
class WC_FOMO_Discount_Generator
{
    ...
    public function admin_page()
    {
        ...
        $table_exists = $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}wcfd_campaigns'") == $wpdb->prefix . 'wcfd_campaigns';
        if (!$table_exists) {
            $this->create_tables();
        }
            ...
            <div class="wcfd-header-actions"
                ...
                <div style="display: flex; gap: 10px;">
                    <form method="post" style="margin: 0;">
                        ...
                        <input type="submit" name="wcfd_check_updates" class="button button-secondary"
                            value="🔄 Check for Updates" />
                    </form>
                    <form method="post" style="margin: 0;">
                        <?php wp_nonce_field('wcfd_cleanup_test_data', 'wcfd_cleanup_nonce'); ?>
                        <input type="submit" name="wcfd_cleanup_test_data" class="button button-secondary"
                            value="🗑️ Clear Test Data"
                            onclick="return confirm('Are you sure? This will delete all claimed codes and WooCommerce coupons for testing.');" />
                    </form>
                </div>
            </div>
                            ...
                            <tr id="tier_config_row" style="display:none;">
                                ...
                                <td>
                                    <div id="tier_builder">
                                        ...
                                        <div class="tier-row">
                                            <label>First <input type="number" name="tier_1_codes" min="1" placeholder="50"
                                                    style="width:60px;"> codes get <input type="number" name="tier_1_discount"
                                                    step="0.01" placeholder="25" style="width:60px;"><span
                                                    class="tier-1-suffix">%</span> discount</label>
                                        </div>
                                        <div class="tier-row">
                                            <label>Next <input type="number" name="tier_2_codes" min="1" placeholder="30"
                                                    style="width:60px;"> codes get <input type="number" name="tier_2_discount"
                                                    step="0.01" placeholder="15" style="width:60px;"><span
                                                    class="tier-2-suffix">%</span> discount</label>
                                        </div>
                                        <div class="tier-row">
                                            <label>Remaining codes get <input type="number" name="tier_3_discount" step="0.01"
                                                    placeholder="10" style="width:60px;"><span class="tier-3-suffix">%</span>
                                                discount</label>
                                        </div>
                                        <p class="description">Example: First 50 codes = 25% off, next 30 codes = 15% off,
                                            remaining = 10% off</p>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <?php foreach ($campaigns as $campaign): ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="#" class="button button-secondary wcfd-delete-campaign"
                                            data-id="<?php echo $campaign->id; ?>"
                                            data-name="<?php echo esc_attr($campaign->campaign_name); ?>" style="color: #d63384;">
                                            Delete
                                        </a>
                                    </td>
                                </tr>
    ...
    public function emails_page()
    {
        ...
        <div class="wrap">
            ...
            <div class="wcfd-stats-grid">
                ...
                <div class="wcfd-stat-card">
                    <h3><?php echo $stats->total_emails > 0 ? round(($stats->used_codes / $stats->total_emails) * 100, 1) : 0; ?>%
                    </h3>
            ...
            <div class="wcfd-email-controls">
                ...
                <div class="wcfd-export-buttons">
                    ...
                    <?php if (!empty($emails)): ?>
                        <button id="wcfd-bulk-delete" class="button button-secondary" style="color: #d63384; margin-left: 10px;">
                            🗑️ <?php _e('Bulk Delete', 'wc-fomo-discount'); ?>
                        </button>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        <th class="check-column"><input type="checkbox" id="wcfd-select-all"></th>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($emails as $email): ?>
                            <tr>
                                <td class="check-column"><input type="checkbox" class="wcfd-lead-checkbox"
                                        data-id="<?php echo $email->id; ?>"></td>
                                ...
                                <td>
                                    <a href="#" class="button button-small wcfd-delete-lead" data-id="<?php echo $email->id; ?>"
                                        data-email="<?php echo esc_attr($email->user_email); ?>" style="color: #d63384;">
                                        Delete
                                    </a>
                                </td>
                            </tr>
            ...
            <?php if ($total_pages > 1): ?>
                <div class="tablenav bottom">
                    <div class="tablenav-pages">
                        <span
                            class="displaying-num"><?php printf(__('%s items', 'wc-fomo-discount'), number_format($total)); ?></span>
                        <?php
    ...
    public function email_settings_page()
    {
        ...
        <div class="wrap">
            ...
            <!-- Current Status -->
            <div class="wcfd-email-status"
                style="margin: 20px 0; padding: 15px; border-radius: 5px; <?php echo $using_smtp ? 'background: #d4edda; border: 1px solid #c3e6cb;' : 'background: #fff3cd; border: 1px solid #ffeaa7;'; ?>">
                ...
                <p style="margin: 0; color: <?php echo $using_smtp ? '#155724' : '#856404'; ?>;">
                    ...
                    <?php else: ?>
                        Emails are being sent using WordPress's default mail() function. <strong>This may result in emails being
                            flagged as spam or not delivered.</strong> Configure SMTP below for better deliverability.
                    <?php endif; ?>
                </p>
            </div>
            ...
            <form method="post" action="">
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">Enable this to use SMTP instead of PHP's mail() function for better
                                deliverability.</p>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <input type="text" name="smtp_host" value="<?php echo esc_attr($smtp_settings['host']); ?>"
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="number" name="smtp_port" value="<?php echo esc_attr($smtp_settings['port']); ?>"
                                class="small-text" />
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="smtp_encryption">
                                ...
                                <option value="tls" <?php selected($smtp_settings['encryption'], 'tls'); ?>>TLS (recommended)
                                </option>
                            </select>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="smtp_username" value="<?php echo esc_attr($smtp_settings['username']); ?>"
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="password" name="smtp_password"
                                value="<?php echo esc_attr($smtp_settings['password']); ?>" class="regular-text"
                                placeholder="<?php echo $smtp_settings['password'] ? '••••••••' : 'Your SMTP password'; ?>" />
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="email" name="from_email" value="<?php echo esc_attr($smtp_settings['from_email']); ?>"
                                class="regular-text" />
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="from_name" value="<?php echo esc_attr($smtp_settings['from_name']); ?>"
                                class="regular-text" />
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="password" name="github_token" value="<?php echo esc_attr($github_token); ?>"
                                class="regular-text"
                                placeholder="<?php echo $github_token ? '••••••••••••••••' : 'ghp_xxxxxxxxxxxxxxxxxxxx'; ?>" />
                            <p class="description">Required only for private repositories. <a
                                    href="https://github.com/settings/tokens" target="_blank">Generate a Personal Access
                                    Token</a> with 'repo' permissions.</p>
                            <p class="description"><strong>Public repositories don't need this.</strong></p>
                        </td>
                    </tr>
                </table>

                <div class="wcfd-smtp-providers"
                    style="margin-top: 20px; padding: 20px; background: #f9f9f9; border-radius: 5px;">
                ...
                <p class="submit">
                    ...
                    <input type="submit" name="wcfd_test_email" class="button-secondary" value="Send Test Email"
